...
public class CrudRepositoryTestRunner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		try {
			...
			System.out.println("___________________");
			System.out.println("count of records ::" + list.stream().count()); // java 8 streaming api for Aggragate
																				// operations
			System.out.println("___________________");
			System.out.println("Sorted records");
			list.stream().sorted(Comparator.comparing(actor -> actor.getAid())).forEach(System.out::println); // java 8
																												// streaming
																												// api
																												// for
																												// sorting
			System.out.println("_______________________");
			list.stream().sorted((a1, a2) -> a1.getAname().compareTo(a2.getAname())).forEach(System.out::println); // java
																													// 8
																													// steaming
																													// api
																													// for
			...
			List<String> namesList = list.stream().map(actor -> actor.getAname()).collect(Collectors.toList()); // java
																												// 8
																												// stream
																												// api
																												// for
...
//			service.fetchActorsByIds(List.of(9, 10, 51)).forEach(System.out::println); // forEach(-) method + method
//																						// reference
			System.out.println("------------------------------------");
...
//				System.out.println("Actor Info::" + opt3.get());
			...
			Actor act = new Actor(9, "Ranveer", "Star Hero", 77777999L);
			System.out.println(service.updateActor(act));

			System.out.println("____________Save or Update object operation____________");
			Actor actor2 = new Actor(9, "Ranveer", "super Star", 7777999L);
			...
			Actor actor3 = new Actor(10, null, null, null);
			System.out.println(service.removeActor(actor3));
